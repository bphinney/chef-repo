...
if node['af']['content_fallback'] == "true"
  ...
  execute "fstab_check" do
    command "fscheck=`grep /opt/tomcat/contentback /etc/fstab | grep -v #{nfsserver.first}:/content`; if [ \"$fscheck\" != \"\" ]; then umount /opt/tomcat/contentback; sed -i '/opt\\/tomcat\\/contentback/d' /etc/fstab; fi"
    action :run
    only_if "grep /opt/tomcat/contentback /etc/fstab | grep -v #{nfsserver.first}:/content#{node.chef_environment}"
  end
  ...
  execute "remount" do
    command "umount /opt/tomcat/contentback; mount /opt/tomcat/contentback"
    action :run
